...
export const createException = async (
    data: Omit<ExceptionDisponibilite, 'id' | 'employe' | 'type_label' | 'valide' | 'statut' | 'statut_label' | 'date_creation'>
...
export const validerException = async (id: number): Promise<ExceptionDisponibilite> => {
    const res = await api.post(`/exceptions/${id}/valider/`)
    return res.data
}

export const rejeterException = async (id: number): Promise<ExceptionDisponibilite> => {
    const res = await api.post(`/exceptions/${id}/rejeter/`)
    return res.data
}
